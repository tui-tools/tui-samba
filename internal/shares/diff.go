...
// enough to place a changed parameter under its `[share]` header without
// turning a fifty line smb.conf into a wall of text in the confirm dialog.
const diffContext = 2
...
// "everything out, everything in", because the confirm dialog for a share edit
// has one job: show the line that changed. A diff that repeats the whole
// smb.conf every time a parameter moved buries exactly that.
//
// The files here are an smb.conf or a drop-in of a few dozen lines, so the
